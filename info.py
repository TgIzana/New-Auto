...
# ============================
SESSION = environ.get('SESSION', 'ɪsʜɪᴛᴀ')
API_ID = int(environ.get('API_ID', '26345065'))
API_HASH = environ.get('API_HASH', '4a22b1377b155165cf3e645125dcfab5')
BOT_TOKEN = environ.get('BOT_TOKEN', "7469847639:AAFbJMEhH6KSeSVykBNtotiTBtpZJ240pnQ")
...
# ============================
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '7726254379').split()] # Replace with the actual admin ID(s) to add
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1002148045188').split()]  # Channel id for auto indexing (make sure bot is admin)
LOG_CHANNEL = int(environ.get('LOG_CHANNEL', '-1002580500985'))  # Log channel id (make sure bot is admin)
BIN_CHANNEL = int(environ.get('BIN_CHANNEL', '-1002681172701'))  # Bin channel id (make sure bot is admin)
MOVIE_UPDATE_CHANNEL = int(environ.get('MOVIE_UPDATE_CHANNEL', '-1002616672491'))  # Notification of those who verify will be sent to your channel
PREMIUM_LOGS = int(environ.get('PREMIUM_LOGS', '-1002687094851'))  # Premium logs channel id
auth_channel = environ.get('AUTH_CHANNEL', '-1002569746645')  # Channel/Group ID for force sub (make sure bot is admin)
DELETE_CHANNELS = [int(dch) if id_pattern.search(dch) else dch for dch in environ.get('DELETE_CHANNELS', '-1002833548896').split()]
support_chat_id = environ.get('SUPPORT_CHAT_ID', '-1002157965776')  # Support group id (make sure bot is admin)
reqst_channel = environ.get('REQST_CHANNEL_ID', '-1002602900862')  # Request channel id (make sure bot is admin)
AUTH_CHANNEL = [int(fch) if id_pattern.search(fch) else fch for fch in environ.get('AUTH_CHANNEL', '-1002569746645').split()]
MULTI_FSUB = [int(channel_id) for channel_id in environ.get('MULTI_FSUB', '-1002569746645 -1002157965776').split() if re.match(r'^-?\d+$', channel_id)]  # Channel for force sub (make sure bot is admin)
...
OWNER_UPI_ID = environ.get('OWNER_UPI_ID', 'bhavdikartik@fam')

#Auto approve 
CHAT_ID = [int(app_chat_id) if id_pattern.search(app_chat_id) else app_chat_id for app_chat_id in environ.get('CHAT_ID', '-4964159322').split()]
TEXT = environ.get("APPROVED_WELCOME_TEXT", "<b>{mention},\n\nʏᴏᴜʀ ʀᴇǫᴜᴇsᴛ ᴛᴏ ᴊᴏɪɴ {title} ɪs ᴀᴘᴘʀᴏᴠᴇᴅ.\n\‣ ᴘᴏᴡᴇʀᴇᴅ ʙʏ ғᴀɪᴛʜ-ᴏᴘ</b>")
...
VERIFIED_LOG = int(environ.get('VERIFIED_LOG', '-1002429678636'))  # Log channel id (make sure bot is admin)
...
# ============================
GRP_LNK = environ.get('GRP_LNK', 'https://t.me/aayeinchat')
CHNL_LNK = environ.get('CHNL_LNK', 'https://t.me/aayein17')
OWNER_LNK = environ.get('OWNER_LNK', 'https://t.me/gachan01')
MOVIE_UPDATE_CHANNEL_LNK = environ.get('MOVIE_UPDATE_CHANNEL_LNK', 'https://t.me/moviesall17')
OWNERID = int(os.environ.get('OWNERID', '7726254379'))  # Replace with the actual admin ID
